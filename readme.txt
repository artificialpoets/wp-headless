...
SEO/AEO engine + open module system. Head meta, JSON-LD, and llms.txt are a headless site's machine-readable content channels — this release makes them first-class.

* Module registry — every subsystem is a keyed module, toggleable via `modules.{key}.enabled` and extensible through the `wp_headless_modules` filter; Settings → WP Headless lists them.
* Prerender module — first-paint pre-rendering stored in a dedicated table, generated by the theme's own SSR bundle (`tools/render-pages.mjs` + `dist-server/`), auto-regenerated on save with WP-CLI support (`wp headless prerender`).
* Schema.org `@graph` JSON-LD in the served head.
* `/llms.txt` and `/llms-full.txt` endpoints (llmstxt.org) for AI agents.
* robots.txt AI-crawler policy with a configurable allow/block stance.
* Head cleanup — strips headless-irrelevant WordPress head output.
* Fixed: soft 404s — WordPress's `is_404()` now decides the response status, so paged and empty archives return real 404s.
...
Paged and empty archives now return real 404s (previously soft-404 200s). If a route you expect to work starts returning 404, it was already broken for search engines — fix the route rather than the status. All new modules are on by default and can be disabled per key via `modules.{key}.enabled`.
