...
/**
 * Application Passwords — list / create / revoke.
 *
 * Uses /wp/v2/users/me/application-passwords which WordPress has shipped
 * since 5.6. The REST nonce in runtime.rest.nonce is sufficient when the
 * session cookie is present (the normal logged-in case).
 *
 * @param {{ runtime: object, request: object }} props
 */
export function ProfilePasswords({ runtime, request }) {
  const { user, isLoggedIn } = useAuth(runtime)
  ...
  const [newSecret, setNewSecret] = useState(null) // {uuid, password, name}
  ...
  if (!isLoggedIn) {
    return (
      <div className={styles.wrap}>
        <Breadcrumbs items={[{ label: 'Home', to: '/' }, { label: 'Profile', to: '/profile/' }, { label: 'Application Passwords' }]} />
  ...
  async function handleDelete(uuid) {
    if (!confirm('Revoke this application password? This cannot be undone.')) return
    try {
