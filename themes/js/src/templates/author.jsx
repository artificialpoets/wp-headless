...
/**
 * Author archive — equivalent to WordPress author.php.
 *
 * The queried user (id, display_name, description, avatar) comes from
 * runtime.request.queried_object as serialised by the wp-headless plugin.
 * We then fetch /wp/v2/users/{id} for the richer profile fields when available
 * and the posts authored by that user.
 *
 * @param {{ runtime: object, request: object }} props
 */
export function Author({ runtime, request }) {
  const [params, setParams] = useSearchParams()
  const page = Math.max(1, parseInt(params.get('page') || '1', 10))

  const queried = request?.queried_object
  const authorId = queried?.id
  const { user } = useAuthor(runtime, authorId)

  const name = user?.name ?? queried?.display_name ?? 'Author'
  const description = user?.description ?? queried?.description ?? ''
  const avatar = user?.avatar_urls?.['96'] ?? queried?.avatar
  ...
  const { posts, totalPages, loading } = usePosts(runtime, authorId
