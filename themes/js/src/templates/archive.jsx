...
/**
 * Archive template — equivalent to WordPress archive.php.
 *
 * Handles category, tag, and custom taxonomy archives. Hierarchical taxonomies
 * (parent → child terms) render the parent chain in the breadcrumb trail —
 * `request.queried_object.ancestors` is a root-to-leaf array.
 *
 * @param {{ runtime: object, request: object }} props
 */
export function Archive({ runtime, request }) {
  ...
  const params2 = termId
    ? { per_page: 10, page, [taxonomyToParam(taxonomy)]: termId }
    : null

  const { posts, totalPages, loading } = usePosts(runtime, params2)
...
/** Map taxonomy slug to WP REST API filter parameter. */
function taxonomyToParam(taxonomy) {
