...
export function PostCard({ post, runtime, featured = false }: PostCardProps) {
  ...
  const formatClass = format ? styles[`format-${format}` as keyof typeof styles] : undefined

  return (
    <article className={`${styles.card} ${featured ? styles.featured : ''} ${formatClass ?? ''}`}>
