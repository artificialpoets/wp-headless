...
export function DateArchive({ runtime, request }: DateArchiveProps) {
  const [params, setParams] = useSearchParams()
  const page = Math.max(1, parseInt(params.get('page') || '1', 10))

  const { year, month, day } = request?.date_archive ?? ({} as { year?: number; month?: number; day?: number })
  ...
  const restParams: Record<string, string | number> | null = range
...
function computeRange(year?: number, month?: number, day?: number): { after: string; before: string } | null {
  if (!year) return null
  if (day && month) {
    const date = `${year}-${pad(month)}-${pad(day)}`
    return { after: `${date}T00:00:00`, before: `${date}T23:59:59` }
  }
  if (month) {
...
function formatHeading(year?: number, month?: number, day?: number): string {
  ...
  if (day && month) return `${MONTHS[month]} ${day}, ${year}`
  if (month) return `${MONTHS[month]} ${year}`
  return String(year)
}

function buildCrumbs(year?: number, month?: number, day?: number): BreadcrumbItem[] {
  const out: BreadcrumbItem[] = [{ label: 'Home', to: '/' }]
  if (year) {
