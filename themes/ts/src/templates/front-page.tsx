...
export function FrontPage({ runtime, request }: FrontPageProps) {
  ...
  useHead({
    ...
    siteName: site?.name,
    ogType: 'website',
    ogImage: (site?.logo?.url as string) || undefined,
    rssUrl,
    robots: request?.robots,
    jsonLd: site
      ? ([websiteSchema(site), organizationSchema(site)].filter(Boolean) as object[])
      : undefined,
  })
  ...
  return (
    <div className={styles.wrap}>
      ...
      <section className={styles.hero}>
        {site?.logo?.url && (
          <img src={site.logo.url as string} alt={site?.name || ''} className={styles.heroLogo} />
        )}
